...
const CountrySetup = () => {
    const { countries, pagination, isLoading, error, getCountries, createCountry, updateCountry, deleteCountry, getCountryById, currentCountry } = useCountry();
    ...
    const handleSave = async () => {
        ...
        const loadingToast = toast.loading(editingItem ? 'Updating country...' : 'Creating country...');
        ...
        try {
            ...
            toast.success(editingItem ? 'Country updated successfully' : 'Country created successfully', { id: loadingToast });
            handleCloseDrawer();
            ...
            if (error.response?.data?.errors) {
                ...
                toast.error(error.message || 'Failed to save country', { id: loadingToast });
            }
        }
    };
    ...
    const handleDeleteConfirm = async () => {
        if (deleteConfirm.id) {
            ...
            try {
                await deleteCountry(deleteConfirm.id);
                toast.success('Country deleted successfully', { id: loadingToast });
                getCountries(pagination.currentPage);
                handleDeleteCancel();
            } catch (error) {
                console.error("Failed to delete country:", error);
                toast.error(error.message || 'Failed to delete country', { id: loadingToast });
            }
        }
    };
            ...
            <div className="animate-in fade-in duration-500 pb-8 space-y-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="text-left">
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight uppercase">Country Setup</h1>
                        <p className="text-gray-500 dark:text-gray-400 mt-1 font-medium text-sm">Manage global country entities.</p>
                    </div>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-x-auto min-h-[400px]">
                    <table className="w-full text-left border-collapse min-w-[600px]">
                        <thead>
                            <tr className="bg-gray-50/50 dark:bg-gray-800/30 border-b border-gray-100 dark:border-gray-800">
                                <th className="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest w-1/4">Country Name</th>
                    ...
                    {filteredData.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            <Globe size={48} className="mb-4 text-gray-200 dark:text-gray-700" />
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white uppercase tracking-tight">NO RESULTS AVAILABLE</h3>
                            <p className="text-sm text-gray-500 max-w-[250px] mt-2">No countries match your active search filter.</p>
                        </div>
                    )}
                </div>
                ...
                {pagination && pagination.total > 0 && (
                    <div className="bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">
                            Displaying <span className="text-primary-600 font-bold">{countries.length}</span> of <span className="text-primary-600 font-bold">{pagination.total}</span> Countries
                        </p>
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 mb-6">
                                        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
                                            Country: <span className="font-bold text-primary-600 dark:text-primary-400">{deleteConfirm.name}</span>
                                        </p>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                            This action cannot be undone. This will permanently remove the country entity.
                                        </p>
                                    </div>
                ...
                <div className={cn("absolute inset-x-0 bottom-0 top-0 md:inset-y-0 md:right-0 md:left-auto md:max-w-[520px] w-full bg-white dark:bg-gray-900 shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] flex flex-col", isDrawerOpen ? "translate-x-0" : "translate-x-full")}>
                    <div className="p-8 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between sticky top-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl z-20">
                        <div className="text-left">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className="text-[11px] font-black text-primary-600 uppercase tracking-[0.3em]">Country Config</span>
                            </div>
                            <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">
                                {editingItem ? 'Modify' : 'New'} <span className="text-primary-600">Country</span>
                            </h2>
                        </div>
                    ...
                    <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50 backdrop-blur-md sticky bottom-0 z-10 flex gap-4">
                        ...
                        >
                            ...
                            {editingItem ? 'UPDATE COUNTRY' : 'SAVE COUNTRY'}
                        </button>
                    </div>
                </div>
            </div>
                    ...
                    <div className="p-8 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between sticky top-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl z-20">
                        <div className="text-left">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className="text-[11px] font-black text-primary-600 uppercase tracking-[0.3em]">Country Profile</span>
                            </div>
                            <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">
                                {currentCountry?.name || 'Country'} <span className="text-primary-600">Details</span>
                            </h2>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                        <div className="space-y-6">
                            <div className="bg-gray-50 dark:bg-gray-800/50 p-6 rounded-2xl border border-gray-100 dark:border-gray-800">
                                <div className="space-y-6 text-left">
                                    <div>
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-2 block">Country Name</label>
                                        <p className="text-sm font-bold text-gray-900 dark:text-white uppercase font-mono">{currentCountry?.name || 'N/A'}</p>
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-2 block">Country Code</label>
                                        <p className="text-sm font-bold text-gray-900 dark:text-white uppercase font-mono">{currentCountry?.code || 'N/A'}</p>
                                    </div>
                                    <div>
                    ...
                    <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/80 sticky bottom-0 z-10 flex gap-4">
                        <button
                            onClick={() => {
                                ...
                                handleOpenDrawer(currentCountry);
                            }}
